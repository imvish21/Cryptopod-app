...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const deployedContract = await etherTransactionContract
    .deploy({
  ...
  etherTransactionContract.events
    .EtherTransferred()
    .on("data", (event) => {
    ...
    .on("error", (error) => {
      ...
      alert(
        "Error in Ether transfer. Please check the recipient address and try again."
      );
    });
  ...
  window.transferEther = async () => {
    ...
    try {
      ...
      if (error.message.includes("Error")) {
        alert(
          "Error transferring Ether. Please check the recipient address and try again."
        );
